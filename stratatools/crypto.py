...
class Desx_Crypto(Crypto):
    ...
    def encrypt(self, key, plaintext):
        ...
        des = DES.new(bytes(key[0:8]), DES.MODE_CBC, bytes(bytearray(8))).encrypt
        for i in range(len(plaintext)//8):
            ciphertext[i*8:i*8+8] = strxor(bytes(output_whitening_key), des(strxor(bytes(input_whitening_key), bytes(plaintext[i*8:i*8+8]))))
    ...
    def decrypt(self, key, ciphertext):
        ...
        des = DES.new(bytes(key[0:8]), DES.MODE_CBC, bytes(bytearray(8))).decrypt
        for i in range(len(ciphertext)//8):
            plaintext[i*8:i*8+8] = bytearray(strxor(bytes(input_whitening_key), des(strxor(bytes(output_whitening_key), bytes(ciphertext[i*8:i*8+8])))))
            des = DES.new(bytes(key[0:8]), DES.MODE_CBC, bytes(bytearray(8))).decrypt
