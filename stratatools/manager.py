...
class Manager:
    ...
    def unpack(self, cartridge_packed):
        ...
        # Manufacturing lot
        manufacturing_lot = struct.unpack_from("<20s", cartridge_packed, 0x10)[0].split(b'\x00')[0]
    ...
    def decrypt(self, machine_number, eeprom_uid, cartridge_crypted):
        ...
        if self.checksum.checksum(cartridge_crypted[0x00:0x40]) != struct.unpack("<H", bytes(cartridge_crypted[0x46:0x48]))[0]:
