...
// Permute the low `inWidth` bits of `in` according to a 1-indexed table
// (bit 1 = MSB of the inWidth-bit value), producing nOut bits.
static uint64_t permute(uint64_t in, const int *table, int nOut, int inWidth) {
    uint64_t out = 0;
...
static uint64_t bytesToU64(const uint8_t *b, int n) {
    uint64_t v = 0;
    for (int i = 0; i < n; i++) v = (v << 8) | b[i];
    return v;
}

static void u64ToBytes(uint64_t v, uint8_t *b, int n) {
...
void des_key_schedule(const uint8_t key[8], uint64_t subkeys[16]) {
    ...
    uint64_t permuted = permute(k, PC1, 56, 64);
...
static uint32_t feistel(uint32_t R, uint64_t subkey) {
    uint64_t x = permute(R, E, 48, 32) ^ subkey;   // 48 bits
    uint32_t out = 0;
    for (int i = 0; i < 8; i++) {
        uint8_t six = (uint8_t)((x >> (42 - 6 * i)) & 0x3F);
...
void des_process_block(const uint64_t subkeys[16], const uint8_t in[8],
    ...
    uint64_t block = permute(bytesToU64(in, 8), IP, 64, 64);
    uint32_t L = (uint32_t)((block >> 32) & 0xFFFFFFFF);
    ...
    u64ToBytes(permute(pre, FP, 64, 64), out, 8);
}
